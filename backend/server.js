...
app.use(cors({
  // Fix 3: always validate against allowedOrigins regardless of env
  origin: (origin, cb) => {
...
app.use((req, _res, next) => {
  ...
  req.body   = clean(req.body)   // Fix 2: sanitise body too
  next()
})
...
app.get('/ready', async (_req, res) => {
  ...
  // Fix 1: always clean up the Redis client via finally block
  {
    ...
    try {
      ...
      await redisClient.quit().catch(() => {}) // always release, even if connect failed
    }
  }
